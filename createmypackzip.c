...
int main(int argc , char *argv[]){
  ...
  int Plus = 1+ strlen("+ Tipo: ")+1+strlen(" Compresion: ")+1+strlen(" Tamano: ")+(strlen(sizeS)*2)+strlen(" TamanoComp: ")+strlen(" Nombre: ")+strlen(comp.FileInfo.DataFileName)+strlen(" DataPos: ")+1+strlen(" +");
  for (i = 0; i < Plus; i++)
  {
    write(fichS,"+",1);
  }
