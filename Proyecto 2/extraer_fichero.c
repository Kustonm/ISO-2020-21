...
int main(int argc[], char *argv[])
{
    ...
    strcpy(texto,read(fichE,buff,1));
    ...
    if (strstr(texto, "Nombre: ") != NULL)
    {
        ...
        while (read(fichE, buff, 1) > 0)
        {
            if (buff != " ")
            {
                strncat(fS.FileInfo.DataFileName,buff,1);;
            }
        }
    }
    ...
    fS=listC.vHead[sizeof(listC.vHead)];
    
    if (sizeof(listC) > argv[2])
    {
        write(fichS, "Tipo: ", strlen("+ Tipo: "));
        write(fichS, &listC.vHead[Index].FileInfo.Type, 1);
        write(fichS, " Compresion: ", strlen(" Compresion: "));
        write(fichS, &listC.vHead[Index].FileInfo.Compress, 1);
        write(fichS, " Tamano: ", strlen(" Tamano: "));
        write(fichS, listC.vHead[Index].FileInfo.DataSize, strlen(listC.vHead[Index].FileInfo.DataSize));
        write(fichS, " TamanoComp: ", strlen(" TamanoComp: "));
        write(fichS, listC.vHead[Index].FileInfo.CompSize, strlen(listC.vHead[Index].FileInfo.DataSize));
        write(fichS, " Nombre: ", strlen(" Nombre: "));
        write(fichS, listC.vHead[Index].FileInfo.DataFileName, strlen(listC.vHead[Index].FileInfo.DataFileName));
        write(fichS, " DataPos: ", strlen(" DataPos: "));
        write(fichS, listC.vHead[Index].FileInfo.DatPosition, strlen(listC.vHead[Index].FileInfo.DatPosition));
        write(fichS,"\n",1);
        write(fichS, listC.vHead[Index].Dummy, strlen(listC.vHead[Index].Dummy));
    }
